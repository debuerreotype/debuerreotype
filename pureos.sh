...
suite="${1:-green}" # http://repo.pureos.net/pureos/dists/
...
pureosDockerImage="$dockerImage-pureos"
[ -z "$build" ] || docker build -t "$pureosDockerImage" - <<-EODF
	FROM $dockerImage
	RUN wget -O pureos.deb 'http://repo.pureos.net/pureos/pool/main/p/pureos-archive-keyring/pureos-archive-keyring_2016.09_all.deb' \\
		&& apt install -y ./pureos.deb \\
		&& rm pureos.deb \\
		&& for d in green purple landing; do \\
	             wget https://source.puri.sm/pureos/core/debootstrap/raw/master/scripts/\$d -O /usr/share/debootstrap/scripts/\$d; \\
		   done
EODF
...
docker run \
	...
	bash -Eeuo pipefail -c '
		...
		exportDir="output"
		outputDir="$exportDir/pureos/$dpkgArch/$suite"
...

		{
			debuerreotype-init --non-debian \
				--debootstrap-script /usr/share/debootstrap/scripts/green \
				--keyring /usr/share/keyrings/pureos-archive-keyring.gpg \
				--include pureos-archive-keyring \
				--include gpg \
				--exclude debian-archive-keyring \
				--merged-usr \
				rootfs "$suite" "$mirror"
			echo "deb $mirror $suite main" | tee rootfs/etc/apt/sources.list
			...
			debuerreotype-apt-get rootfs install -y --no-install-recommends iputils-ping iproute2
